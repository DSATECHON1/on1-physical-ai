...
def get_canonical_fingerprint(mission):
    """
...
def build_konnex_submission_package(mission):
    ...
    physical_hardware = adapter_integration.get(
        "physicalHardware",
        False,
    )
    ...
    checks["notSubmittedToKonnex"] = (
        submitted_to_konnex is False
    )

    checks["notKonnexVerified"] = (
        konnex_verified is False
    )

    checks["notOnChainVerified"] = (
        on_chain_verified is False
    )

    checks["physicalHardwareNotClaimed"] = (
        physical_hardware is False
    )

    checks["evidenceArtifactPresent"] = (
        bool(evidence_artifact)
    )

    artifact_sha256 = evidence_artifact.get(
        "sha256"
    )
